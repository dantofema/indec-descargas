...
### DES-Q3 · Qué pasa con los padres sintéticos cuando el objeto no existe → **(a) la fila de padres no se dibuja** · **cerrada 2026-09-12 → `DES-R12`**
...
### DES-Q2 · Quién decide qué tipos entran al catálogo → **(a) `DES-R6` absorbe la membresía** · **cerrada 2026-09-12 → `DES-R6`**
...
| **DES-R6** | **El catálogo se regenera a mano, se versiona, y lleva los cinco tipos que se buscan por nombre.** `npm run build:index` lo produce; el archivo se commitea. **Qué lleva adentro:** jurisdicción, departamento, localidad censal, gobierno local y aglomerado. Fracción, radio y vía **no entran**: se direccionan por código (BUS-R5) y se recorren como capas hijas, pero no se buscan por nombre. **Por qué esos cinco y no los ocho:** medido el 2026-09-12, el archivo pesa 658 KB para 6.977 objetos, 97 bytes cada uno. Los 557.651 objetos de los ocho tipos proyectan unos 51 MB, que el home tendría que bajar para que alguien escriba tres letras. Y los tres que quedan afuera no tienen nombre propio que buscar: un radio censal se llama por su código. **Por qué:** el Marco Geoestadístico cambia entre censos, no entre semanas. Versionarlo hace que el deploy no dependa de que el GeoServer del INDEC esté arriba, y que un cambio en los datos se vea en el diff. | ✅ **implementada 2026-09-04** |
...
| **DES-R12** | **Si el objeto de la ficha no existe, la fila de padres no se dibuja.** La cadena de un objeto sin catálogo se arma cortando su código (DES-R10), así que la de uno que no existe apunta a objetos que tampoco: decir «no existe» y abajo ofrecer su fracción con el botón activo es contradecirse en la misma pantalla. **Lo que se acepta:** DES-R8 documenta que los códigos del INDEC no cierran entre capas —529 departamentos en una, 530 en otra—, así que un padre escondido acá **puede** existir, y quien lo necesitaba pierde esa salida. Se elige igual: ofrecer un enlace que probablemente no lleva a nada, al lado de un mensaje que dice que el objeto no está, enseña a desconfiar de los dos. **Por qué:** es la misma decisión que DES-R11, un nivel más arriba. La ficha dice una sola cosa sobre lo que tiene delante. *Mutación aplicada antes de marcarla:* no esconder los padres pone en rojo su caso; esconderlos siempre rompe el caso del objeto que sí existe y la mitad de DES-R10. | ✅ **implementada 2026-09-12** · #6 |
